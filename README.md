...
단어 앱은 이미 넘친다. 그런데 대부분이 실패하는 지점은 카드 품질이나 SRS 알고리즘이 아니라, **사람들이 앱을 "열지 않는다"는 것**이다. 아무리 좋은 알고리즘도 매일 앱을 켜야 작동한다.

그래서 popVOCA는 문제를 다시 정의했다 — *"더 좋은 단어 앱"이 아니라, "앱을 열 필요 자체를 없애는 것."*

- **🔒 잠금화면 학습** — 폰을 켤 때마다(하루 수십 번) 단어 카드가 먼저 뜬다. 공부하러 "들어가는" 게 아니라, 이미 보고 있는 화면에 공부가 온다.
- **🪟 플로팅 오버레이** — 유튜브를 보든 카톡을 하든, 그 위에 미니 카드를 얹는다. 학습이 다른 행동을 방해하지 않고 스며든다.

즉 학습을 **사용자가 이미 있는 자리로** 가져온다. 이게 popVOCA의 핵심 가설이고, 이 경험을 위해 앱 밖에서 동작하는 Kotlin 네이티브 모듈까지 직접 구현했다.

## 핵심 디자인 결정

- **2,640단어 → "20단어 = 1걸음, 132걸음"** — 방대한 목록의 압박을 지우고 매일의 진척을 눈에 보이게 했다. 목표는 완주가 아니라 "오늘 한 걸음."
- **체크 트랙 vs 정복 트랙** — "본 단어"와 "아는 단어"를 분리했다. 플래시카드를 끝내면 걸음이 *체크*(다음 걸음 해금), 퀴즈까지 통과해야 *정복* — 진도의 착시를 막는다.
- **6가지 퀴즈 유형** — 재인(4지선다)부터 재생(스펠링)까지 인출 강도를 달리해, 같은 단어를 다른 근육으로 반복하게 했다.
- **오버레이·잠금화면을 네이티브로** — 학습을 앱 밖으로 꺼내려면 React Native만으론 불가능했다. `SYSTEM_ALERT_WINDOW` 오버레이와 잠금화면 리시버를 Kotlin 로컬 모듈로 직접 구현했다.
